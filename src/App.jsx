    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
      <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-md animate-fade-in">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-800">HENCE 출장 관리 시스템</h1>
        ...
        <div className="flex-1 overflow-auto border rounded-lg">
          <table className="w-full text-sm text-left">
            <thead className="bg-gray-100 text-gray-700 sticky top-0">
              <tr>
                <th className="p-3">이름</th><th className="p-3">아이디</th><th className="p-3">비밀번호</th><th className="p-3">권한</th><th className="p-3 text-center">관리</th>
              </tr>
            </thead>
              ...
              {users.map(u => (
                <tr key={u.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-3 text-center">
                    {u.id !== 'admin' && <button onClick={() => handleDeleteUser(u.id)} className="text-red-500 hover:bg-red-50 p-1 rounded"><Trash2 size={16}/></button>}
                  </td>
                </tr>
              ))}
            </tbody>
...
export default function App() {
  ...
  const handleDelete = () => {
    if(confirm('정말 삭제하시겠습니까?')) {
      saveReportsToStorage(reports.filter(r => r.id !== selectedReport.id));
      alert('삭제되었습니다.');
      setView('dashboard');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans print:bg-white">
      ...
      <main className="w-full max-w-[1920px] mx-auto p-4 md:p-8 print:p-0">
          ...
          <div className="animate-fade-in space-y-8">
            ...
            <div className="flex justify-between items-center">
              <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
                {user.role === 'admin' ? <><UserCog className="text-blue-600"/> 관리자 대시보드</> : <><FileText className="text-blue-600"/> 나의 보고서 관리</>}
              </h2>
              <div className="flex gap-2">
                {user.role === 'admin' && (
                ...
                {user.role === 'employee' && (
                  <button onClick={startWrite} className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 font-bold flex items-center gap-2 shadow-md transition">
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
              <div className="p-5 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                <h3 className="text-lg font-bold text-gray-700 flex items-center gap-2">
                  <FileText size={20} className="text-gray-500"/> {user.role === 'admin' ? '전체 제출된 보고서' : '나의 제출 이력'}
                </h3>
                <span className="text-sm text-gray-500">총 {(user.role === 'admin' ? reports : reports.filter(r => r.userId === user.id)).length}건</span>
              </div>
                    ...
                    {(user.role === 'admin' ? reports : reports.filter(r => r.userId === user.id)).map((report) => (
                      <tr key={report.id} className="hover:bg-blue-50 transition-colors group">
                        ...
                        <td className="px-6 py-4 text-center">
                          <button 
                            onClick={() => { setSelectedReport(report); setView('preview'); }}
                            className="bg-white border border-blue-200 text-blue-600 px-3 py-1 rounded hover:bg-blue-50 font-bold text-xs transition shadow-sm"
                          >
                            상세보기
                          </button>
                        </td>
          ...
          <div className="bg-gray-100 min-h-screen p-4 md:p-8 print:bg-white print:p-0">
            ...
            {selectedReport.status === 'rejected' && selectedReport.rejectionReason && (
              <div className="max-w-[210mm] mx-auto mb-6 bg-red-50 border border-red-200 p-4 rounded-lg flex items-start gap-3 print-hidden">
                ...
                <div>
                  <h4 className="font-bold text-red-700">관리자 반려 사유</h4>
                  <p className="text-red-600 text-sm mt-1">{selectedReport.rejectionReason}</p>
                </div>
              </div>
            )}
            ...
            {selectedReport.resubmissionComment && (
              <div className="max-w-[210mm] mx-auto mb-6 bg-blue-50 border border-blue-200 p-4 rounded-lg flex items-start gap-3 print-hidden">
                <MessageSquare className="text-blue-600 flex-shrink-0 mt-1"/>
                <div>
                  <h4 className="font-bold text-blue-700">작성자 재기안 코멘트</h4>
                  <p className="text-blue-600 text-sm mt-1">{selectedReport.resubmissionComment}</p>
                </div>
              </div>
            )}

            {/* A4 미리보기 컨테이너 (기존 디자인 유지) */}
            <div className="a4-preview-container print-container relative">
              {/* 승인 도장 */}
              {selectedReport.status === 'approved' && (
                <div className="absolute top-10 right-10 border-4 border-green-600 text-green-600 rounded-full w-32 h-32 flex items-center justify-center transform rotate-[-15deg] opacity-80 pointer-events-none">
                  <div className="text-center">
                    <div className="text-sm font-bold border-b border-green-600 pb-1 mb-1">{new Date().toLocaleDateString()}</div>
                    <div className="text-2xl font-black">승 인</div>
                    <div className="text-xs font-bold mt-1">HENCE 관리자</div>
                  </div>
                </div>
              )}
              {/* 반려 도장 */}
              {selectedReport.status === 'rejected' && (
                <div className="absolute top-10 right-10 border-4 border-red-600 text-red-600 rounded-full w-32 h-32 flex items-center justify-center transform rotate-[-15deg] opacity-80 pointer-events-none">
                  <div className="text-2xl font-black">반 려</div>
                </div>
              )}
              ...
              <div className="flex justify-end mb-8 text-center text-sm">
                <table className="border-collapse border border-gray-400">
                  <tbody>
                    <tr><td className="border border-gray-400 bg-gray-100 p-1 w-20">담당</td><td className="border border-gray-400 bg-gray-100 p-1 w-20">팀장</td><td className="border border-gray-400 bg-gray-100 p-1 w-20">부서장</td></tr>
                    <tr>
                      <td className="border border-gray-400 h-20 align-middle font-bold text-gray-400">{selectedReport.travelerName.slice(0,1)}인</td>
                      <td className="border border-gray-400 h-20"></td>
                      <td className="border border-gray-400 h-20 align-middle text-green-600 font-bold">{selectedReport.status === 'approved' ? '승인' : ''}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              ...
              <table className="w-full border-collapse border border-gray-400 mb-6 text-sm">
                <tbody>
                  <tr>
                    <td className="border border-gray-400 bg-gray-100 p-3 font-bold w-1/5 text-center">출장 기간</td>
                    <td className="border border-gray-400 p-3 w-2/5">{selectedReport.startDate} ~ {selectedReport.endDate}</td>
                    <td className="border border-gray-400 bg-gray-100 p-3 font-bold w-1/5 text-center">출장자</td>
                    <td className="border border-gray-400 p-3 w-1/5 text-center">{selectedReport.travelerName} 외 {selectedReport.travelerCount - 1}명</td>
                  </tr>
                  <tr><td className="border border-gray-400 bg-gray-100 p-3 font-bold text-center">출장 장소</td><td className="border border-gray-400 p-3" colSpan="3">{selectedReport.location}</td></tr>
                  <tr><td className="border border-gray-400 bg-gray-100 p-3 font-bold text-center">출장 목적</td><td className="border border-gray-400 p-3" colSpan="3">{selectedReport.purpose}</td></tr>
                </tbody>
              </table>
              ...
              <div className="mb-6">
                ...
                <div className="border border-gray-400 p-4 min-h-[150px] whitespace-pre-wrap text-sm leading-relaxed">{selectedReport.content}</div>
              </div>
              ...
              <div className="mb-6">
                ...
                <table className="w-full border-collapse border border-gray-400 text-sm text-center">
                  ...
                  <tbody>
                    <tr>
                      <td className="border p-2">교통/유류비</td>
                      <td className="border p-2 text-left">{selectedReport.vehicleType === 'personal' ? `개인차량 (${selectedReport.distance}km * ${FUEL_RATE_PERSONAL}원)` : `법인차량 (${selectedReport.distance}km)`}</td>
                      <td className="border p-2 text-right">{formatCurrency(selectedReport.fuelCost)}</td>
                      <td className="border p-2 text-gray-500">{selectedReport.vehicleType === 'personal' ? '' : '법인카드'}</td>
                    </tr>
              ...
              {selectedReport.receipts.length > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {selectedReport.receipts.map(r => (<div key={r.id} className="border p-2 text-center break-inside-avoid"><img src={r.url} className="max-h-60 mx-auto object-contain"/><p className="text-xs mt-1 text-gray-500">{r.name}</p></div>))}
                  </div>
                </div>
              )}
              ...
              <div className="mt-12 text-center">
                ...
                <p className="mt-2 font-bold">신청자 : {selectedReport.travelerName} (인)</p>
              </div>
            </div>
          </div>
        )}
        ...
        {showResubmitModal && <ResubmitCommentModal onClose={() => setShowResubmitModal(false)} onConfirm={(comment) => { setShowResubmitModal(false); if(confirm('수정된 내용을 제출하시겠습니까?')) submitReport(comment); }} />}
        ...
        {rejectionNotification && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4 backdrop-blur-sm">
            <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-sm border-l-4 border-red-500 animate-bounce-in">
              ...
              <p className="mb-4">
                <strong>{new Date(rejectionNotification.submittedAt).toLocaleDateString()}</strong>에 제출한 보고서가 반려되었습니다.
              </p>
              <div className="bg-red-50 p-3 rounded text-sm text-red-700 mb-4">
                <strong>사유:</strong> {rejectionNotification.rejectionReason}
              </div>
