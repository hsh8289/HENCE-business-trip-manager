...
import { Calendar, Users, MapPin, FileText, DollarSign, Car, Upload, Printer, Trash2, Plus, Save } from 'lucide-react';
...
export default function App() {
  ...
  const handleExpenseChange = (id, field, value) => {
    setFormData(prev => ({
      ...
      expenses: prev.expenses.map(e => 
        e.id === id ? { ...e, [field]: field === 'amount' ? parseInt(value) || 0 : value } : e
      )
    }));
  };
  ...
  const handleFileUpload = (e) => {
    ...
    if (file) {
      ...
      setFormData(prev => ({
        ...prev,
        receipts: [...prev.receipts, { id: Date.now(), name: file.name, url: imageUrl }]
      }));
    }
  };
  ...
  const calculateTotal = () => {
    const expenseTotal = formData.expenses.reduce((sum, item) => sum + item.amount, 0);
      ...
      <header className="bg-blue-600 text-white p-4 shadow-md print:hidden">
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <button onClick={handleSave} className="flex items-center gap-1 px-3 py-1 bg-blue-500 hover:bg-blue-400 rounded text-sm transition">
              <Save size={16} /> 임시저장
            </button>
            <button onClick={() => setActiveTab('report')} className={`px-4 py-2 rounded font-medium transition ${activeTab === 'report' ? 'bg-white text-blue-600' : 'bg-blue-700 text-white'}`}>
              작성하기
            </button>
            <button onClick={() => setActiveTab('preview')} className={`px-4 py-2 rounded font-medium transition ${activeTab === 'preview' ? 'bg-white text-blue-600' : 'bg-blue-700 text-white'}`}>
              미리보기/인쇄
            </button>
          </div>
        </div>
        ...
        {activeTab === 'report' && (
          <div className="space-y-6 animate-fade-in print:hidden">
            ...
            <div className="flex justify-center pt-6">
              <button onClick={() => setActiveTab('preview')} className="bg-blue-600 text-white px-8 py-3 rounded-lg font-bold text-lg hover:bg-blue-700 shadow-lg transition">
                작성 완료 및 미리보기
              </button>
            </div>
          </div>
        ...
        {(activeTab === 'preview' || activeTab === 'print') && (
          <div className="bg-white shadow-2xl print:shadow-none min-h-[29.7cm] p-8 md:p-12 print:p-0">
            ...
            <div className="flex justify-between items-center mb-8 print:hidden border-b pb-4">
              <button onClick={() => setActiveTab('report')} className="text-gray-600 hover:text-gray-900 underline">
                &larr; 수정하기
              </button>
              <button onClick={handlePrint} className="bg-gray-800 text-white px-4 py-2 rounded flex items-center gap-2 hover:bg-black transition">
            ...
            <div className="max-w-3xl mx-auto border border-gray-400 p-8 print:border-0 print:p-0 print:w-full">
              ...
              <table className="w-full border-collapse border border-gray-400 mb-6 text-sm">
                <tbody>
                  <tr>
                    ...
                    <td className="border border-gray-400 p-2 w-2/5">{formData.startDate} ~ {formData.endDate}</td>
                    <td className="border border-gray-400 bg-gray-100 p-2 font-bold w-1/5">출장자</td>
                    <td className="border border-gray-400 p-2 w-1/5">{formData.travelerName} 외 {formData.travelerCount - 1}명</td>
                  </tr>
                  <tr>
                    <td className="border border-gray-400 bg-gray-100 p-2 font-bold">출장 장소</td>
                    <td className="border border-gray-400 p-2" colSpan="3">{formData.location}</td>
                  </tr>
                  <tr>
                    <td className="border border-gray-400 bg-gray-100 p-2 font-bold">출장 목적</td>
                    <td className="border border-gray-400 p-2" colSpan="3">{formData.purpose}</td>
                  </tr>
                </tbody>
              </table>
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td className="border border-gray-400 p-2 text-left">
                        {formData.vehicleType === 'personal' ? `개인차량 (${formData.distance}km * ${FUEL_RATE_PERSONAL}원)` : `법인차량 (${formData.distance}km)`}
                      </td>
                      <td className="border border-gray-400 p-2 text-right">{formatCurrency(formData.fuelCost)}</td>
                    ...
                    <tr className="bg-gray-50 font-bold">
                      ...
                      <td className="border border-gray-400 p-2 text-right text-blue-600">{formatCurrency(calculateTotal())}</td>
                      <td className="border border-gray-400 p-2"></td>
                    </tr>
                  </tbody>
                </table>
              ...
              {formData.receipts.length > 0 && (
                <div className="break-before-auto">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {formData.receipts.map(receipt => (
                      <div key={receipt.id} className="border border-gray-300 p-2">
              ...
              <div className="mt-12 text-center">
                ...
                <p className="mt-2 font-bold">신청자 : {formData.travelerName} (인)</p>
              </div>
